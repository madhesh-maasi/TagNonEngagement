...
export default class SubmitSplitReviews extends React.Component<ISubmitSplitReviewsProps, SubmitSplitReviewsState, {}> {
  ...
  private ServiceLineOptions: IDropdownOption[] = [
    { key: 'Capital Markets & Accounting Advisor', text: 'Capital Markets & Accounting Advisor' },
  ...
  constructor(props: any) {
    super(props);
    this.state = {
      AppContext: this.props.context, IsLoading: true, hasEditItemPermission: false,
      SplitReviews: new TAG_SplitReviews(),
      IsCreateMode: (this.props.ItemID == undefined || this.props.ItemID == null || this.props.ItemID == 0) ? true : false, DisableSaveButton: true
    };
  ...
  public async componentDidMount() {
    ...
    if (this.state.IsCreateMode) { }
      ...
      this.listSplitReviewsItemService = new ListItemService(this.props.context, Config.ListNames.SplitReviews);
      this.hasEditItemPermission = await this.listSplitReviewsItemService.CheckCurrentUserCanEditItem(this.props.ItemID);
      const SplitReviewsDetails: TAG_SplitReviews = await this.listSplitReviewsItemService.getItemUsingCAML(this.props.ItemID, [], undefined, Enums.ItemResultType.TAG_SplitReview);
  ...
  private redirect() {
    let returnURL = this.props.context.pageContext.web.absoluteUrl + Config.Links.HomePageLink;
  ...
  private async onSave(): Promise<void> {
    ...
    if (this.state.IsCreateMode) {
      ...
      this.listSplitReviewsItemService = new ListItemService(this.props.context, Config.ListNames.SplitReviews);
      await this.listSplitReviewsItemService.updateItem(this.props.ItemID, data);
      this.redirect();
    }
  ...
  private validateSave(updateDetails: TAG_SplitReviews): boolean {
    ...
    if (updateDetails.Title != "" && updateDetails.Title != undefined &&
      updateDetails.SourceReviewID != "" && updateDetails.SourceReviewID != undefined &&
        ...
        <div className={styles.container}>
          ...
          <div className={styles.lblTopText}>
            <div className={styles.divCompetency}>
              <Label><b><span style={{ color: '#ff0000' }}>BEFORE YOU BEGIN: </span></b>Identify the ID number of the review you would like to split into an additional review. You may split any unstarted review into an additional review, even if it has been split before. Consult the lists below for reviews you are eligible to split.</Label>
            </div>
            <div className={styles.divCompetency}>
            ...
            <div className={styles.divCompetency}>
              <Label>If you need additional assistance, please contact the TAG Performance Administrators. </Label>
            </div>
          ...
          <div className={styles.row}>
            <div className={styles.lblSourceReviewID}> <Label><b>Source Review ID (Choose from below): <span style={{ color: '#ff0000' }}>*</span></b></Label></div>
            <div className={styles.txtSourceReviewID}> <TextField resizable={false} multiline={false} value={this.state.SplitReviews.SourceReviewID} onChange={this.onChangeSourceReviewID} ></TextField>   </div>

            <div className={styles.lblHourstoReview}> <Label><b>Hours to Review </b></Label></div>
            <div className={styles.txtSourceReviewID}><TextField resizable={false} multiline={false} value={this.state.SplitReviews.HourstoReview} onChange={this.onChangeHourstoReview} ></TextField>   </div>
          </div>

          <div className={styles.row}>
            <div className={styles.lblSourceReviewID}>
              <Label className={styles.lblText}><b>Title of new Split Review</b> <br /><b>(Example: Acme Software Implementation - Phase 1) </b></Label>
            </div>
            <div className={styles.txtSourceReviewID}> <TextField resizable={false} multiline={false} value={this.state.SplitReviews.Title} onChange={this.onChangeTitleofnewSplitReview} ></TextField>   </div>
          </div>
          <div className={styles.row}>

            <div className={styles.lblSourceReviewID}> <Label><b>Service Line <span style={{ color: '#ff0000' }}>*</span></b></Label></div>
            <div className={styles.txtSourceReviewID}>
              <Dropdown placeholder="Please Select a value" selectedKey={this.state.SplitReviews.ServiceLine}
                onChange={(e, selectedOption) => { this.onChangeServiceLine(selectedOption.text); }}
                options={this.ServiceLineOptions} /></div>

           
          </div>
          <div className={styles.row}>
          <div className={styles.lblSourceReviewID}> <Label><b>Job Title <span style={{ color: '#ff0000' }}>*</span></b></Label></div>
            <div className={styles.txtSourceReviewID}>
              <Dropdown placeholder="Please Select a value" selectedKey={this.state.SplitReviews.JobTitle}
                onChange={(e, selectedOption) => { this.onChangeJobTitle(selectedOption.text); }}
                options={this.JobTitleOptions} /></div>
          </div>
