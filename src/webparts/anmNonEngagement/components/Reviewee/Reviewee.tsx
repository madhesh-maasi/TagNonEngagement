...
export default class Reviewee extends React.Component<IAnmNonEngagementProps, {}> {
    ...
    public async componentDidMount() {
        ...
        const ReviewTypeState = [{ value: 'Mid Year', label: 'Mid Year' }, { value: 'Year End', label: 'Year End' }];


        this.webUrl = await sp.site.getWebUrlFromPageUrl(window.location.href);
        console.log('props', this.props);

        this.listItemService = new ListItemService(this.props.AppContext, Config.ListNames.FiscalYears);

        const allFiscalYear: TAG_GenericDetails[] = await this.listItemService.getItemsUsingCAML([Config.BaseColumns2.Title], Config.BaseColumns2.SortOrder,
            '', 100, Enums.ItemResultType.TAG_FiscalYear);

        this.listItemService = new ListItemService(this.props.AppContext, Config.ListNames.NonEngReview);
        const allChoiceRoles: string[] = await this.listItemService.getFieldChoices(Config.NonEngagementReviewTemplateColumns.Role);
       ...
       // const userRoles: Enums.UserRoles[] = await this.GetCurrentUserRoles();
        ...
        let CurrentUserIsAdmin: boolean = await this.userService.CheckCurrentUserIsFullControl();
        this.setState({
            IsAdmin: CurrentUserIsAdmin,
            ReviewTypeOptions: ReviewTypeState,
            allFiscalYear: newState, allRoles: newallRoles,
            currentuser: this.props.user, mentor: mentor,
            selectedFYYear: { value: "" + new Date().getFullYear(), label: new Date().getFullYear() }
        });
    }
    private async onChangeRevieweeName(items: any[]) {

        if (items != null && items.length > 0) {
            let RevieweeName = await MapResult.map(items[0], Enums.MapperType.PnPControlResult, Enums.ItemResultType.User);
            ...
            <div className={styles.anmNonEngagement}>
                {this.state["showAssigneeForm"] && <div>
                    <div className='titlebar'>
                        ...
                        <span> To initiate a review,
                            indicate the Fiscal Year, Role and then click on</span>
                        <span className={styles.boldlabel}> Start Reivew</span>
                    </div>

                    {(this.state["IsAdmin"]) &&
                        <div className={styles.AdminTag}>
                            <span className={styles.boldlabelAdminTag} >Non Engagement Annual Review - Admin</span>
                        </div>
                    }
                    ...
                    <div className='mainContainer'>
                        <div className='row mt30'>
                            <div className='col-md-6'>
                                {(!this.state["IsAdmin"]) &&
                                    <div className='form-controls'>
                                        ...
                                        <label className='text-left'>{this.props.user ? this.props.user.Title : ''}</label>
                                    </div>
                                }
                                {(this.state["IsAdmin"]) &&
                                    <div className='form-controls'>
                                        ...
                                        <div className={styles.divPeoplePicker}>
                                            {<PeoplePicker
                                                ...
                                                defaultSelectedUsers={[this.state['RevieweeNameEmail']]}
                                                resolveDelay={1000} />}
                                        </div>
                                    </div>
                                }
                            ...
                            <div className='col-md-6'>
                                <div className='form-controls'>
                                    ...
                                    <Dropdown options={this.state["allFiscalYear"]}
                                        onChange={e => { this.setState({ selectedFYYear: e["value"] }); }}
                                        // onChange={this.onChange}
                                        value={this.state["selectedFYYear"]}
                                        placeholder="Select an option" /></div></div>
                            ...
                            <div className='col-md-6'>
                                <div className='form-controls'>
                                    ...
                                    <Dropdown
                                        ...
                                        onChange={e => { this.setState({ selectedReviewType: e["value"] }); }}
                                        value={this.state["selectedReviewType"]}
                                        placeholder="Select an option" /></div></div>


                            <div className='col-md-6'>
                                <div className='form-controls'><label className={styles.boldlabel}>Role:</label>
                                    <Dropdown options={this.state["allRoles"]}
                        ...
                        <div className='dflex mt1 jc'>
                            ...
                            <button className='btn btn-primary'
                                ...
                                disabled={this.state["selectedRole"] == "" || this.state["selectedFYYear"] == "" || this.state["selectedReviewType"] == ""}
                            >Start Review</button>

                            <button className='btn btn-outline-dark ml1' onClick={() => this.cancel()}>Close</button>
                        </div>
                    </div>
                </div>}
    ...
    private saveData = async () => {
        ...
        let data = {};
        let fyYear = this.state["selectedFYYear"].value ? this.state["selectedFYYear"].value : this.state["selectedFYYear"];
        const columns = Config.NonEngagementReviewTemplateColumns;
        data[columns.FiscalYear] = this.state["selectedFYYear"].value ? this.state["selectedFYYear"].value : this.state["selectedFYYear"];
        ...
        if (this.state["IsAdmin"]) {
            ...
            if (this.state["selectedReviewType"] == "Year End") {
                data[columns.Title] = this.state["RevieweeName"].Title + " - Year End " + fyYear;
            }
            else {
        ...
        data[columns.SignoffHistory] = "Reviewee Initiated - " + this.props.user.Title + " , " + formattedDate;
