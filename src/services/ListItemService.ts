...
export default class ListItemService extends ContextService {
  ...
  public async getProjectItemUsingCAML(
    RevieweeID: number,
    selectFields: string[],
    orderByXML: string,
    resultType: Enums.ItemResultType
    ...
    let camlFilterConditions =
      ...
      "<Eq>" +
      "<FieldRef Name='" +
      Config.ProjectListColumns.RevieweeName +
      "' LookupId='TRUE' />" +
      "<Value Type='Lookup'>" +
      RevieweeID +
      "</Value>" +
  ...
  public async getProjectItemUsingCAMLStatusOfReviewSplit(
    RevieweeID: number,
    selectFields: string[],
    orderByXML: string,
    resultType: Enums.ItemResultType
  ...
  public async getProjectItemUsingCAMLAllProjectStatusSplit(
    RevieweeID: number,
    selectFields: string[],
    orderByXML: string,
    resultType: Enums.ItemResultType
  ...
  public async CheckEmployeeAlreadyExists(employee: User): Promise<boolean> {
    ...
    const allResultItems: any = await this.getItemsUsingCAML(
      ...
      camlFilterConditions,
      1,
      Enums.ItemResultType.TAG_ItemID
    );
  ...
  public async GetMentorOfEmployee(employee: User): Promise<User> {
    ...
    const allResultItems: any = await this.getItemsUsingCAML(
      ...
      camlFilterConditions,
      1,
      Enums.ItemResultType.TAG_MentorDetails
    );
  ...
  public async getAllVersion(itemId, listName) {
    let versionHistory = await sp.web.lists
      .getByTitle(listName)
  ...
  public async getAllFilteresData(listName, filter) {
    ...
    let versionHistory = await sp.web.lists
      .getByTitle("Questionnaire")
      .items.filter("Role eq '" + filter + "'")
      .get();
    return versionHistory;
  }
